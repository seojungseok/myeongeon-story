...
- **테마 집중(`--focus`) 옵션(2026-08-08)**: `--focus longing,love`처럼 슬러그/한글 라벨(쉼표구분)을 주면 그 테마 명언만 골라옴("이별"/"헤어짐"→그리움(longing)으로 매핑, 이별·재회·첫사랑·짝사랑 정서 포함). 드리프트 시 카테고리를 focus 안으로 클램프.
- **⭐ 단계별 자동 생성(2026-08-08)**: 워크플로 `Plan today's batch` 스텝이 `src/content/stories`의 **longing+love 편수**를 세어 그날 배치를 자동 결정 — **1단계**(100편 미만): 하루 **10편, focus=longing,love**. **2단계**(100편 이상): 하루 **5편, 전체 주제**. 사람 손 없이 자동 전환. 하루 1회(06:00 KST)만 몰아 생성(커밋·배포 1회). 수동 실행 시 `count`(비우면 자동)·`focus`(auto=단계자동 / none=전체 / 슬러그 지정)로 덮어쓰기.
