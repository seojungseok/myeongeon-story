...
### 오늘 한 일 — 매일 자동 글 생성 "실제 가동" + 버그 수정
- **모델 확정: `gemini-3-flash-preview`** (가성비). 서버로 사용가능 모델 조회해 3.x flash/pro·`*-latest` 확인. pro는 비싸서 3.0 flash 선택.
- **핵심 발견**: 이전 "드리프트"의 상당수는 **내 테스트가 Git Bash에서 한글을 깨진 인코딩으로 서버에 보낸 탓**. UTF-8 파일로 보내니 3.0 flash가 첫 시도에 명언 그대로 착지. 실제 자동화(quotes.txt UTF-8)는 문제 없음.
- **첫 글 수동 발행**: `buddha-hatred-melts-with-love` (서버 키로 생성, 오늘자 최신글).
- **버그 2개 수정 (안 잡았으면 자동화가 매일 깨졌음)**:
  1. 🔴 **한글 id 페이지 404** (Next.js 정적 라우트가 한글 URL 매칭 실패) → **영문 슬러그 id로 전환**. 프롬프트에 `slug`(영문 kebab) 필드 추가, `generate-story.ts`가 그걸 id로 사용(폴백: 명언 해시). `c174fcf`
  2. 🔴 **자동 글 OG 이미지가 빌드를 깨뜨림** (`RangeError: DataView` — 깨진 캐시 이미지/폰트) → OG를 **요청 시(런타임) 생성**으로 바꿔 빌드가 어떤 콘텐츠에도 안 깨지게. `5a18a4b`
  - JSON 파서도 견고화(모델이 JSON 뒤 잡소리 붙여도 첫 완결 객체만 추출). `92ea5f5`
- **자동화 실전 검증 완료**: GitHub Actions 수동 실행 → **10편 자동 생성·커밋·배포 성공**(`90c9bc3`). 모두 명언 착지·작가 정확·영문 URL. **사이트 총 16편.** `GEMINI_API_KEY` 시크릿 정상 확인됨.
- **명언 풀 대폭 확장** (`scripts/quotes.txt` ~75개): 검색량 높은 "○○ 명언"(니체·쇼펜하우어·공자·노자·부처·톨스토이·헤세·아우렐리우스·간디·아인슈타인 등) + 인간관계·위로·인생 주제. 하루 10개씩 몇 주치 확보. 소진되면 계속 추가.

### 자동화 현재 상태 (완성)
- 매일 **06:00 KST** GitHub Actions가 자동 생성→커밋→Vercel 배포. 컴퓨터 꺼도 됨.
- `gemini-3-flash-preview` + **검증 게이트**(명언 착지·작가·길이 실패시 재시도3, 그래도 실패면 스킵) → 이상한 글 발행 안 됨.
- **SEO 최적화 내장**: 제목 키워드/인물 앞배치, 글마다 고유 description, 태그, 영문 슬러그 URL. (prompt-template.txt)
- 이미 쓴 명언 skip → 매일 새 글.

### 다음(회사에서) 이어서
- 명언 풀 더 채우기(SEO 트래픽 주제). 원하면 100~200개 추가.
- 자동 생성글 가끔 훑어보고 품질 이상하면 프롬프트/검증 조이기.
- (선택) 방문자 카운터 리셋, 네이버 사이트맵 제출.
